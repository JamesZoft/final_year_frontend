...
public class DropDownListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(MatchActivity.checkSelected[position])
	...
	private void setText(int position1){
		if (!MatchActivity.checkSelected[position1]) {
			MatchActivity.checkSelected[position1] = true;
			selectedCount++;
		} else {
			MatchActivity.checkSelected[position1] = false;
			selectedCount--;
		}
		...
		if (selectedCount == 0) {
			...
			for (int i = 0; i < MatchActivity.checkSelected.length; i++) {
				if (MatchActivity.checkSelected[i] == true) {
